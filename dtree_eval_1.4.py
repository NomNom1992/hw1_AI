...
def evaluatePerformance():
	...
	** Note that your implementation must follow this API**
	...
	percents = np.arange(0.1, 0.3 0.1)
	...
	for i in range(100):
		...
		for percent in percents:
			...
			mean_acc["DecisionTree"].append(np.mean(dt_scores))
			std_dt_acc.append(np.std(dt_scores))
			
			mean_acc["DecisionTree"].append(np.mean(ds_scores))
			std_ds_acc.append(np.std(ds_scores))
			
			mean_acc["DecisionTree"].append(np.mean(dt3_scores))
